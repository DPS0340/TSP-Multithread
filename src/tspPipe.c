...
int TSP_child(int *resultPath, int *pathRecord, int *localSmallest, int sum, int threadNumber, int count,
    ...
    *ptr = INT16_MAX;
    ...
    for (int next = 0; next < fileLength; next++)
    {
        ...
        if (visited & (1 << next))
            continue;
        // 재귀적으로 호출하면서 최소값을 찾음
        *ptr = min(*ptr, TSP_child(resultPath, pathRecord, localSmallest, sum + map[currentIndex][next],
